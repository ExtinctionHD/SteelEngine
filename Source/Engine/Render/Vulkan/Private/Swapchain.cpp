...
namespace Details
{
        ...
        const std::vector<vk::SurfaceFormatKHR>& formats, const std::vector<vk::Format>& preferredFormats)
    {
        ...
        for (const auto& preferredFormat : preferredFormats)
        {
            ...
            const auto it = std::ranges::find_if(
                formats, [&](const auto& surfaceFormat)
    ...
    static vk::Extent2D SelectExtent(
        const vk::SurfaceCapabilitiesKHR& capabilities, const vk::Extent2D& requiredExtent)
    {
        ...
        if (capabilities.currentExtent.width == std::numeric_limits<uint32_t>::max())
        {
            ...
            swapchainExtent.height = std::clamp(requiredExtent.height, capabilities.minImageExtent.height, capabilities.maxImageExtent.height);
        }
        else
        {
    ...
    static SwapchainData CreateSwapchain(const Swapchain::Description& description)
    {
        ...
        const vk::SurfaceFormatKHR format
            = SelectFormat(device.GetSurfaceFormats(surface.Get()), preferredFormats);

        const uint32_t minImageCount
            = std::min(capabilities.maxImageCount, VulkanConfig::kSwapchainMinImageCount);
        ...
        const vk::SwapchainCreateInfoKHR createInfo({}, surface.Get(), minImageCount, format.format, format.colorSpace, extent, 1, vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eStorage, SelectSharingMode(device.GetQueuesDescription()), uniqueQueueFamilyIndices, SelectPreTransform(capabilities), SelectCompositeAlpha(capabilities), presentMode, false, nullptr);
            ...
            VulkanContext::device->ExecuteOneTimeCommands(
                [&image](vk::CommandBuffer commandBuffer)
                {
                    const ImageLayoutTransition layoutTransition{
                        vk::ImageLayout::eUndefined, vk::ImageLayout::ePresentSrcKHR, PipelineBarrier::kEmpty
                    };
        ...
        const std::vector<vk::Image>& images, vk::Format format)
    {
        ...
        for (const auto& image : images)
        {
            vk::ImageViewCreateInfo createInfo({}, image, vk::ImageViewType::e2D, format, ImageHelpers::kComponentMappingRGBA, ImageHelpers::kFlatColor);
