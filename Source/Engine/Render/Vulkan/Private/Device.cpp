...
namespace Details
{
    ...
    static std::optional<uint32_t> FindCommonQueueFamilyIndex(vk::PhysicalDevice physicalDevice, vk::SurfaceKHR surface)
    {
        const auto queueFamilies = physicalDevice.getQueueFamilyProperties();
    ...
    static std::vector<vk::DeviceQueueCreateInfo> CreateQueuesCreateInfo(const Queues::Description& queuesDescription)
    {
    ...
    static vk::PhysicalDeviceFeatures2 GetPhysicalDeviceFeatures(const Device::Features& deviceFeatures)
    {
        ...
        using FeaturesStructureChain = vk::StructureChain<vk::PhysicalDeviceFeatures2,
                ...
                vk::PhysicalDeviceScalarBlockLayoutFeatures, vk::PhysicalDeviceRayQueryFeaturesKHR>;

        static FeaturesStructureChain featuresStructureChain(vk::PhysicalDeviceFeatures2(features),
                accelerationStructureFeatures, rayTracingPipelineFeatures, descriptorIndexingFeatures,
                bufferDeviceAddressFeatures, scalarBlockLayoutFeatures, rayQueryFeatures);
    ...
    static Device::RayTracingProperties GetRayTracingProperties(vk::PhysicalDevice physicalDevice)
    {
        ...
        const Device::RayTracingProperties rayTracingProperties{ rayTracingPipelineProperties.shaderGroupHandleSize,
...
{
    const auto physicalDevice = Details::FindSuitablePhysicalDevice(VulkanContext::instance->Get(), requiredExtensions);
    ...
    const vk::DeviceCreateInfo createInfo({}, static_cast<uint32_t>(queueCreatesInfo.size()), queueCreatesInfo.data(),
            0, nullptr, static_cast<uint32_t>(requiredExtensions.size()), requiredExtensions.data(), nullptr);
