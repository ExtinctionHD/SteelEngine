...
namespace Details
{
    ...
    static vk::ImageCreateInfo GetImageCreateInfo(const ImageDescription& description)
    {
        ...
        const vk::ImageCreateInfo createInfo{
            GetVkImageCreateFlags(description.type),
            GetVkImageType(description.type),
            description.format,
            description.extent,
    ...
    static vk::ImageView CreateView(vk::Image image, vk::ImageViewType viewType, vk::Format format,
    ...
    else
    {
        ...
        for (const auto& imageUpdate : imageUpdates)
        {
            const uint32_t expectedSize =
                    Details::CalculateDataSize(imageUpdate.extent, imageUpdate.layers.layerCount, description.format);
        ...
        commandBuffer.copyBufferToImage(stagingBuffer, image, vk::ImageLayout::eTransferDstOptimal, copyRegions);
    }
}
