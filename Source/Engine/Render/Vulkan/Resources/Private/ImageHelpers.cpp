...
Texture ImageHelpers::CreateRenderTarget(
        vk::Format format, const vk::Extent2D& extent, vk::SampleCountFlagBits sampleCount, vk::ImageUsageFlags usage)
{
    ...
    const vk::Image image = VulkanContext::imageManager->CreateImage(imageDescription, ImageCreateFlags::kNone);

    const vk::ImageSubresourceRange subresourceRange = IsDepthFormat(format) ? kFlatDepth : kFlatColor;

    const vk::ImageView view = VulkanContext::imageManager->CreateView(image, vk::ImageViewType::e2D, subresourceRange);
...
void ImageHelpers::GenerateMipLevels(vk::CommandBuffer commandBuffer, vk::Image image, const vk::Extent3D& extent,
