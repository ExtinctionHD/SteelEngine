...
namespace Details
{
    ...
    static vk::AccelerationStructureBuildSizesInfoKHR GetBuildSizesInfo(vk::AccelerationStructureTypeKHR type, const vk::AccelerationStructureGeometryKHR& geometry, uint32_t primitiveCount)
    {
        const vk::AccelerationStructureBuildGeometryInfoKHR buildInfo(type, GetBuildFlags(type), vk::BuildAccelerationStructureModeKHR::eBuild, nullptr, nullptr, 1, &geometry, nullptr, vk::DeviceOrHostAddressKHR(), nullptr);
    ...
    {
        const BufferDescription bufferDescription{ size,
...
vk::AccelerationStructureKHR AccelerationStructureManager::GenerateBlas(const BlasGeometryData& geometryData)
{
    ...
    const vk::Buffer vertexBuffer
        = BufferHelpers::CreateBufferWithData(bufferUsage, ByteView(geometryData.vertices));
    const vk::Buffer indexBuffer
        = BufferHelpers::CreateBufferWithData(bufferUsage, ByteView(geometryData.indices));

    const vk::AccelerationStructureGeometryTrianglesDataKHR trianglesData(geometryData.vertexFormat, VulkanContext::device->GetAddress(vertexBuffer), geometryData.vertexStride, geometryData.vertexCount - 1, geometryData.indexType, VulkanContext::device->GetAddress(indexBuffer), nullptr);

    const vk::AccelerationStructureGeometryKHR geometry(
        vk::GeometryTypeKHR::eTriangles, trianglesData, vk::GeometryFlagsKHR());
    ...
    const vk::AccelerationStructureCreateInfoKHR createInfo(
        {}, buffers.storageBuffer, 0, buildSizesInfo.accelerationStructureSize, type, vk::DeviceAddress());

    const auto [result, blas] = VulkanContext::device->Get().createAccelerationStructureKHR(createInfo);

    const vk::AccelerationStructureBuildGeometryInfoKHR buildInfo(type, vk::BuildAccelerationStructureFlagBitsKHR::ePreferFastTrace, vk::BuildAccelerationStructureModeKHR::eBuild, nullptr, blas, 1, &geometry, nullptr, VulkanContext::device->GetAddress(buffers.scratchBuffer));
...
vk::AccelerationStructureKHR AccelerationStructureManager::GenerateTlas(const TlasInstances& instances)
{
    ...
    VulkanContext::device->ExecuteOneTimeCommands(
        ...
        {
            BufferHelpers::UpdateBuffer(commandBuffer, buffers.sourceBuffer, GetByteView(instances), SyncScope::kWaitForNone, SyncScope::kAccelerationStructureWrite);
        });
    ...
    const vk::AccelerationStructureGeometryKHR geometry(
        vk::GeometryTypeKHR::eInstances, geometryData, vk::GeometryFlagBitsKHR::eOpaque);
    ...
    const vk::AccelerationStructureBuildGeometryInfoKHR buildInfo(type, Details::GetBuildFlags(type), vk::BuildAccelerationStructureModeKHR::eBuild, nullptr, tlas, 1, &geometry, nullptr, VulkanContext::device->GetAddress(buffers.scratchBuffer));
...
void AccelerationStructureManager::BuildTlas(
    vk::CommandBuffer commandBuffer, vk::AccelerationStructureKHR tlas, const TlasInstances& instances)
{
    constexpr vk::AccelerationStructureTypeKHR type = vk::AccelerationStructureTypeKHR::eTopLevel;
    ...
    const PipelineBarrier pipelineBarrier{ SyncScope::kAccelerationStructureWrite, SyncScope::kShaderRead };

    BufferHelpers::InsertPipelineBarrier(commandBuffer, buffers.storageBuffer, pipelineBarrier);
}

void AccelerationStructureManager::DestroyAccelerationStructure(
