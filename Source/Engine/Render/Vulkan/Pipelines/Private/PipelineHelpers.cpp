...
glm::uvec3 PipelineHelpers::CalculateWorkGroupCount(
    const vk::Extent3D& extent, const glm::uvec3& workGroupSize)
{
...
glm::uvec3 PipelineHelpers::CalculateWorkGroupCount(
    const vk::Extent2D& extent, const glm::uvec2& workGroupSize)
{
    return CalculateWorkGroupCount(
        VulkanHelpers::GetExtent3D(extent), glm::uvec3(workGroupSize.x, workGroupSize.y, 1));
}
