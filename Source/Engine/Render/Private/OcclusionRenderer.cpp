...
namespace Details
{
    ...
    static constexpr vk::Viewport kViewport(
        0.0f, 0.0f, static_cast<float>(kExtent.width), static_cast<float>(kExtent.height), 0.0f, 1.0f);

    static constexpr float zNear = 0.001f;

    static Texture CreateDepthTexture()
    {
        const Texture texture = ImageHelpers::CreateRenderTarget(kDepthFormat, kExtent, vk::SampleCountFlagBits::e1, vk::ImageUsageFlagBits::eDepthStencilAttachment);
    ...
    {
        const RenderPass::AttachmentDescription attachment{ RenderPass::AttachmentUsage::eDepth,
            kDepthFormat,
        ...
        const RenderPass::Description description{
            vk::PipelineBindPoint::eGraphics, vk::SampleCountFlagBits::e1, { attachment }
        };
    ...
    static vk::Framebuffer CreateFramebuffer(const RenderPass& renderPass, vk::ImageView imageView)
    {
        ...
        return VulkanHelpers::CreateFramebuffers(device, renderPass.Get(), kExtent, {}, { imageView })
            .front();
    }
    ...
    static glm::mat4 ComputeViewProj(const AABBox& bbox, int32_t directionAxis)
    {
        ...
        const CameraLocation location{ position, direction, up };

        const CameraProjection projection{ 0.0f, width, height, zNear, zFar };
...
bool OcclusionRenderer::ContainsGeometry(const AABBox& bbox) const
{
    for (int32_t i = 0; i < 3; ++i)
    {
        ...
        VulkanContext::device->ExecuteOneTimeCommands(
            ...
            {
                BufferHelpers::UpdateBuffer(commandBuffer, cameraBuffer, GetByteView(viewProj), SyncScope::kWaitForNone, SyncScope::kVertexUniformRead);
