...
namespace Details
{
    ...
    void AddStorageOffset(Scene& scene, const StorageRange& storageRange)
    {
        ...
        {
            const int32_t offset = static_cast<int32_t>(storageRange.textureSamplers.offset);
    ...
    void SubtractStorageRange(Scene& scene, const StorageRange& storageRange)
    {
        if (auto* msc = scene.ctx().find<MaterialStorageComponent>())
        {
            ...
            {
                MaterialHelpers::SubtractTextureRange(material, storageRange.textureSamplers);
            }
        }
    ...
    {
        if (storageRange.textureSamplers.offset >= removedRange.textureSamplers.GetEnd())
        {
            storageRange.textureSamplers.offset -= removedRange.textureSamplers.size;
        }
